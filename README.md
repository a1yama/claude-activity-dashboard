...
- **トークン使用量チャート**: 日別の入力/出力/キャッシュトークン（サブエージェント含む）
- **モデル別利用状況**: モデルごとのメッセージ数・トークン量
- **カスタムコマンド使用ランキング**: 自作スキル/コマンドの利用頻度（`/exit` などのビルトインとサブエージェントは除外）
- **ツール使用ランキング**: どのツール（Bash, Read, Edit 等）が多く使われているか
- **プロジェクト別サマリー**: 直近30日の作業時間・トークン・エラー/セッション
- **最近のセッション一覧**: 直近のセッションへのリンク（最初のユーザー発話を要約として表示）
- **今週の改善候補**: 週次自動生成の提案。採否は Mac 側で記録する

```bash
make proposals-list        # 改善候補と採否の一覧
make proposal-adopt ID=3   # 採用として記録
make proposal-reject ID=3  # 却下として記録
make proposal-reopen ID=3  # 未対応に戻す
```

> 本番の DB は読み取り専用マウントで Mac から上書きされるため、採否は Mac 側で記録し `make sync` で反映する。
...
Playwright を使用してフロントエンド + Datasette API の結合テストを行います。テスト用のフィクスチャDB を自動生成し、専用ポート（Datasette:8766, Vite:5174）で独立したサーバーを起動します。

テスト対象:
- ダッシュボードページ（統計カード、チャート、データ更新ボタン）
- セッション詳細ページ（メッセージ一覧、統計、エラー表示）
- ページ間ナビゲーション
...
│   ├── create-fixture-db.py  # フィクスチャDB生成
